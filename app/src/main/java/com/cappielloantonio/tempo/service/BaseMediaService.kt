...
import com.cappielloantonio.tempo.util.StreamingCacheDataSource
...
open class BaseMediaService : MediaLibraryService() {
    ...
    open fun getMediaLibrarySessionCallback(): MediaLibrarySession.Callback = BaseSessionCallback(baseContext, this)
            ...
            object : Player.Listener {
                ...
                override fun onTimelineChanged(
                    ...
                    for (i in 0 until timeline.windowCount) {
                        ...
                        window.mediaItem.mediaMetadata.artworkUri
                            ?.let { bitmapLoader.prewarm(it) }
                    }
                }
                                ...
                                val browserFuture =
                                    MediaBrowser
                                        .Builder(
                                            ...
                                            SessionToken(
                                                this@BaseMediaService,
                                                ComponentName(this@BaseMediaService, this@BaseMediaService::class.java),
                                            ),
                                        ).buildAsync()
                ...
                override fun onPositionDiscontinuity(
                    ...
                    if (reason == Player.DISCONTINUITY_REASON_AUTO_TRANSITION) {
                        if (oldPosition.mediaItem
                                ?.mediaMetadata
                                ?.extras
                                ?.getString(PlexMediaMapper.EXTRA_TYPE) == Constants.MEDIA_TYPE_MUSIC
                        ) {
    ...
    fun getQueueFromPlayer(player: Player): List<MediaItem> = (0..player.mediaItemCount - 1).map(player::getMediaItemAt)
    ...
    override fun onGetSession(controllerInfo: ControllerInfo): MediaLibrarySession = mediaLibrarySession
    ...
    private fun getAdjustedPitch(speed: Float): Float =
        if (Preferences.isPlaybackSpeedManualPitchEnabled()) {
    ...
    private fun initializeMediaLibrarySession(player: Player) {
        ...
        mediaLibrarySession =
            MediaLibrarySession
                .Builder(this, player, getMediaLibrarySessionCallback())
    ...
    private fun getRenderersFactory(): DefaultRenderersFactory {
        val extensionRendererMode =
            if (DownloadUtil.useExtensionRenderers()) {
