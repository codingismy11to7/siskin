...
object CompositeBuildLocks {
     ...
     *
     * Entries do not accumulate across hours: the last thread out of a key
     * removes it. The removal is the two-argument `remove(key, value)`, which
     * only fires if the mapping is still the lock this call acquired -- a
     * thread that removes late cannot take away a lock some newer builder has
     * already installed under the same key. The cost of that late removal is
     * that a racing pair can briefly hold different locks for one key and both
     * run; the loser then finds the file already cached, and the rename
     * backstop covers the rest. Bounding the map is worth that, because a key
     * carries a bucket and every hour mints eight fresh ones.
     */
