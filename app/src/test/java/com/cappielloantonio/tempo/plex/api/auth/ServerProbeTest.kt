...
class ServerProbeTest {
    ...
    private fun connection(uri: String, local: Boolean = true, relay: Boolean = false) =
        Connection().apply {
            this.uri = uri
            this.local = local
            this.relay = relay
    ...
    private fun identityServer() = MockWebServer().apply {
    ...
    private fun hangingServer() = MockWebServer().apply {
    ...
    @Test
    fun adoptsTheFirstConnectionThatAnswers() = runTest {
        ...
        val winner = ServerProbe().bestConnectionUri(
            resource(
    ...
    @Test
    fun fallsBackToRelayWhenEveryDirectConnectionFails() = runTest {
    ...
    @Test
    fun prefersADirectConnectionOverAWorkingRelay() = runTest {
    ...
    @Test
    fun returnsNullWhenNothingAnswers() = runTest {
        val winner = ServerProbe().bestConnectionUri(resource(connection(deadUri())))
        assertNull(winner)
    }

    @Test
    fun returnsNullForAResourceWithNoConnections() = runTest {
    ...
    fun splitsRelayOutOfTheDirectTier() {
        val candidates = ServerProbe.candidates(
            resource(
