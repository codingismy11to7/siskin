 ...
 * row's media id -- takes it on trust as *the* definition of which
...
class CompositeArtCacheTest {
    ...
    fun theScopeIsTheFirstTwoFieldsOfTheCacheFilename() {
        ...
        val file = CompositeArt.cacheFile(context, "machinea", "4", "1980", 100)
    ...
    fun theSectionKeyIsInTheNameSoLibrariesOnOneServerDoNotShareComposites() {
        ...
        val four = CompositeArt.cacheFile(context, "machinea", "4", "1980", 100)
        val nine = CompositeArt.cacheFile(context, "machinea", "9", "1980", 100)
    ...
    fun theMachineIdentifierIsInTheNameSoDifferentServersDoNotShareComposites() {
        ...
        val serverA = CompositeArt.cacheFile(context, "machinea", "4", "1980", 100)
        val serverB = CompositeArt.cacheFile(context, "machineb", "4", "1980", 100)
    ...
    fun aNullMachineIdentifierAndAnUnsafeOneFallBackToTheSameSentinel() {
        ...
        val noIdentifier = CompositeArt.cacheFile(context, null, "4", "1980", 100)
        val unsafeIdentifier =
            CompositeArt.cacheFile(context, "../evil", "4", "1980", 100)
    ...
    fun aNullMachineIdentifierDoesNotCollideWithARealOne() {
        ...
        val noIdentifier = CompositeArt.cacheFile(context, null, "4", "1980", 100)
        val realIdentifier = CompositeArt.cacheFile(context, "abc123def456", "4", "1980", 100)
    ...
    fun theBucketIsInTheNameSoAnHourRollIsAMiss() {
        val now = CompositeArt.cacheFile(context, "machinea", "4", "1980", 100)
        val next = CompositeArt.cacheFile(context, "machinea", "4", "1980", 101)
    ...
    fun evictionKeepsTheTwoLiveBucketsAndDeletesTheRest() {
        ...
        val live = CompositeArt.cacheFile(context, "machinea", "4", "1980", current)
        val previous = CompositeArt.cacheFile(context, "machinea", "4", "1980", current - 1)
        val stale = CompositeArt.cacheFile(context, "machinea", "4", "1980", current - 2)
        val ancient = CompositeArt.cacheFile(context, "machinea", "4", "1970", 0)
        listOf(live, previous, stale, ancient).forEach {
            it.parentFile!!.mkdirs()
            it.writeBytes(byteArrayOf(1))
        }

        CompositeArt.evictStale(context, nowMs)
    ...
    fun evictionLeavesUnrecognisedFilesAloneRatherThanEmptyingTheDirectory() {
        ...
        // cache.
        val stray = CompositeArt.cacheDir(context).resolve("notours.txt")
        stray.parentFile!!.mkdirs()
        stray.writeText("keep me")

        CompositeArt.evictStale(context, 100 * CompositeArtBucket.BUCKET_MS)
